...
def solve_1(input):
    ...
    return max([winning_score(deck) for deck in combat(deck_1, deck_2, False)])

def solve_2(input):
    deck_1 = [int(p) for p in input[0] if not p.startswith("Player")]
    deck_2 = [int(p) for p in input[1] if not p.startswith("Player")]
    return max([winning_score(deck) for deck in combat(deck_1, deck_2, True)])

def winning_score(deck):
    return sum([value*(idx+1) for idx,value in enumerate(deck[::-1])])

def combat(deck_1, deck_2, recursive):
    rounds = list()
    while len(deck_1) > 0 and len(deck_2) > 0:
        ...
        card_1 = deck_1.pop(0)
        card_2 = deck_2.pop(0)
        if not recursive or (len(deck_1) < card_1 or len(deck_2) < card_2):
            if card_1 > card_2:
            ...
            sub_deck_1, sub_deck_2 = combat(deck_1[:card_1], deck_2[:card_2], recursive)
